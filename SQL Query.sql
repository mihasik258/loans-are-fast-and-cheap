...
FROM (
    ...
    HAVING SUM(CASE WHEN п.Плановая_дата_оплаты BETWEEN CAST(date_trunc('week', CURRENT_DATE) AS DATE) + 7 AND CAST(date_trunc('week', CURRENT_DATE) AS DATE) + 13 THEN 1 ELSE 0 END) > 0 -- Подзапрос фильтрует клиентов с выплатами на след. неделю
) v
JOIN crosstab(
...
) AS pd (Клиент TEXT, Пн NUMERIC, Вт NUMERIC, Ср NUMERIC, Чт NUMERIC, Пт NUMERIC, Сб NUMERIC, Вс NUMERIC) USING (Клиент) -- crosstab разворачивает Пн-Вс в столбцы.
ORDER BY v.Клиент;
...
-- запрос 3: статистика займов по месяцам
    -- два полностью одинаковых подзапроса с сортировкой и LIMIT 1.
    -- чтобы достать ФИО сотрудника и результат,
...
-- два подзапроса с LIMIT 1 извлекают сумму и дату самой крупной задолженности агентства.
SELECT кa.Полное_наименование AS Агентство, COUNT(DISTINCT д.id_Договора) AS Кол_договоров, COUNT(DISTINCT з.id_Клиента) AS Уник_должников,
    ...
    FROM Договор_о_сотрудничестве д2 
    JOIN Документ_займа з2 ON д2.id_Займа = з2.id_Займа 
    LEFT JOIN План_выплат п2 ON з2.id_Займа = п2.id_Займа -- находим исходный займ по переданному договору
...
JOIN Договор_о_сотрудничестве д ON кa.id_Агентства = д.id_Агентства -- все дела агентства
