...
def generate():
    ...
    for li in range(N_LOAN):
        ...
        for p in range(np):
            ...
            plan_cnt += 1
    ins("План_выплат", ["ID_Плана", "Плановая_дата_оплаты","Сумма_погашения_основы",
                         "Сумма_погашения_процентов","Сумма_погашения_штрафа",
    ...
    for idx,li in enumerate(trans):
        ...
        ctrs.append((idx+100, f"ДОГ-КОЛ-{idx+100:03d}", rand_date(365,30),
                      round(loans[li][2]*.7,2), round(loans[li][2]*.3,2),
                      random.choice(["В работе","Исполнен"]), li+100,
                      random.randint(1,2), acc, pr))
    ins("Договор_о_сотрудничестве",
        ["ID_Договора", "Регистрационный_номер","Дата_подписания","Сумма_основного_долга",
         "Сумма_накопленных_процентов","Статус_договора","ID_Займа",
